%
% This script classify the features of the dataset. 
...
% should only be in the diagonal), and the fraction of correct classification.

% Add path for PRTools
addpath c:\Users\Gourab\Documents\prtools4.2.5\prtools

clc
clear all

% Directory where the dataset exists
dataDir = 'P2_class/';
d = dir([dataDir 't*']);
...
for i=1:length(d),
	...
	for j=1:2,
		...
		A = imread(name);
		vecTrain((i-1)*2+j,:) = [computeFeatureVector(A),computeTextureFeatureVector(A)]; %training vector
		labTrain((i-1)*2+j) = i+48; %training labels
	end
	for j=3:length(d1),
		name = [dataDir namedir '/' d1(j).name];
		A = imread(name);
		vecTest((i-1)*4+j-2,:) = [computeFeatureVector(A),computeTextureFeatureVector(A)]; %testing vector
